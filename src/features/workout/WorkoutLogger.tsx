    ...
    >
      ...
      >
        <div className="relative z-10 text-center space-y-8">
          ...
          <div>
            <h2 className="text-4xl font-black text-white leading-tight mb-2 uppercase tracking-tighter italic">
              Victory!
            </h2>
      ...
      <div className="relative h-80 bg-zinc-900 rounded-[2.5rem] border border-white/5 overflow-hidden shadow-inner">
        ...
        <div className="absolute bottom-6 left-6 right-6 grid grid-cols-2 gap-4">
          ...
          <div className="bg-black/60 backdrop-blur-xl p-4 rounded-2xl border border-white/5">
            ...
            <p className="text-lg font-black text-white">
              {distance > 0 ? (time / (distance / 1000) / 60).toFixed(2) : '0.00'} /km
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="text-center space-y-2">
        <div className="flex items-baseline justify-center gap-2">
          <input
            type="number"
            value={
              exerciseType === 'walking' || exerciseType === 'footsteps'
                ? Math.round(distance * 1.31)
                : Math.round(distance) || ''
            }
            onChange={(e) => {
        ...
        <p className="text-xs font-bold text-zinc-500 uppercase tracking-[0.2em]">
          Target:{' '}
          {exerciseType === 'walking' || exerciseType === 'footsteps'
            ? Math.round(targetDistance * 1.31)
            : targetDistance}
          {exerciseType === 'walking' || exerciseType === 'footsteps' ? ' steps' : 'm'}
        </p>
      </div>
...
export function WorkoutLogger() {
    ...
    return (
      <section className="max-w-xl mx-auto min-h-screen bg-black text-white flex flex-col items-center justify-center p-6 pb-24 font-sans text-center space-y-8">
        ...
        <div className="space-y-2">
          ...
          <p className="text-zinc-500 font-medium px-8">
            Initialize your daily protocol to begin training and earn XP.
          </p>
        </div>
        <button
  ...
  const progressPercent =
    ((activeEx?.repsLogged || activeEx?.distanceLogged || 0) /
      (activeEx?.targetReps || activeEx?.targetDistance || 1)) *
    100;

  const questCompletion =
    (activeQuest.exercises.filter((ex) => ex.state === 'completed').length /
      activeQuest.exercises.length) *
    100;
    ...
    <section className="max-w-xl mx-auto min-h-screen bg-black text-white flex flex-col p-6 pb-24 font-sans">
      ...
      {view === 'list' ? (
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500 font-bold uppercase tracking-widest">
                  {activeQuest.exercises.length} Exercises Assigned
                </p>
              </div>
            </div>
                ...
                >
                  <div className="flex items-center gap-4">
                    {ex.state === 'completed' ? (
                      ...
                      <div className="w-6 h-6 rounded-full border-2 border-zinc-800 flex items-center justify-center text-[10px] font-black text-zinc-700">
                        {idx + 1}
                      </div>
                    )}
                    <div className="text-left">
                      <p
                        className={`font-black uppercase italic ${ex.state === 'completed' ? 'text-zinc-500 line-through' : 'text-white'}`}
                      >
                      ...
                      <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                        {ex.targetReps
                          ...
                          : (ex.exerciseType as string) === 'walking' ||
                              (ex.exerciseType as string) === 'footsteps'
        ...
        <div className="flex-1 flex flex-col">
          {(activeEx.exerciseType as string).includes('run') ||
          (activeEx.exerciseType as string) === 'walking' ||
          (activeEx.exerciseType as string) === 'footsteps' ? (
            <GpsTracker
              ...
              onComplete={(d) =>
                updateExerciseProgress(
                  activeEx.id,
                  undefined,
                  (activeEx.distanceLogged || 0) + Math.round(d)
                )
              }
            ...
            <div className="flex-1 flex flex-col items-center justify-center space-y-12">
              <div className="relative w-72 h-72 flex items-center justify-center">
                <svg className="absolute inset-0 w-full h-full -rotate-90 scale-110">
                  ...
                  <motion.circle
                    ...
                    animate={{
                      strokeDashoffset: 816.8 - (816.8 * Math.min(100, progressPercent)) / 100
                    }}
                    strokeLinecap="round"
                ...
                <div className="text-center z-10">
                  <p className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.4em] mb-2">
                    Logged
                  </p>
              ...
              <div className="w-full max-w-[280px] bg-zinc-900/50 rounded-[2.5rem] p-8 border border-white/5">
                <MuscleGraphic
                  growth={activeEx.muscleGroups.reduce(
                    (acc, mg) => {
                      acc[mg.name as keyof typeof acc] = activeEx.repsLogged
                        ? (activeEx.repsLogged / (activeEx.targetReps || 1)) * mg.growthPercentage
                        : 0;
                      return acc;
                    },
              ...
              <div className="w-full grid grid-cols-3 gap-3">
                <button
                  onClick={() =>
                    updateExerciseProgress(activeEx.id, (activeEx.repsLogged || 0) + 1)
                  }
                ...
                <button
                  onClick={() =>
                    updateExerciseProgress(activeEx.id, (activeEx.repsLogged || 0) + 5)
                  }
                  className="py-8 rounded-3xl bg-zinc-900 border border-white/5 flex flex-col items-center justify-center gap-2 hover:bg-zinc-800 active:scale-95 transition-all"
                >
