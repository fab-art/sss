...
export function WorkoutLogger() {
  const { activeQuest, updateExerciseProgress, completeActiveQuest, progression, startQuest } = useProgressionStore(useShallow(state => ({
  ...
  // BOLT OPTIMIZATION: Memoize derived state and objects to prevent redundant calculations
  // and re-renders of child components (like MuscleGraphic) during frequent state updates.
  const activeEx = activeQuest?.exercises[activeExIdx];

  const isAllComplete = useMemo(() =>
    activeQuest?.exercises.every(ex => ex.state === 'completed') ?? false
