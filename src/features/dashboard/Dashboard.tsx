...
export function Dashboard({ onStartTraining, onViewNutrition }: DashboardProps) {
  ...
  const { progression, streak, activeQuest, runningProgress, startQuest, syncSteps } =
    useProgressionStore();
  ...
  const nutritionSummary = getSummary(0) as DailyNutritionSummary & {
  ...
  return (
    <section className="max-w-xl mx-auto space-y-8 pb-24">
      ...
      >
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-bold text-primary">
                {getXpRequiredForNextLevel() - getXpIntoLevel(progression.totalXp)} XP
              </p>
            </div>
          </div>
        ...
        <div className="absolute -top-12 -right-8 text-[12rem] font-black text-white/[0.03] pointer-events-none">
          {progression.level}
        </div>
      </motion.div>
        ...
        <div className="rounded-[2rem] bg-zinc-900 border border-white/5 p-6 space-y-4 shadow-xl">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mt-1">
              Goal: {runningProgress.stepGoal?.toLocaleString()}
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <button
              onClick={handleSync}
              className={`flex w-full items-center justify-center gap-1 rounded-xl py-2 text-[10px] font-black uppercase transition ${syncSuccess ? 'bg-primary text-black shadow-[0_0_15px_rgba(34,197,94,0.4)]' : 'bg-primary/10 text-primary hover:bg-primary/20'}`}
              aria-label={
                syncSuccess ? 'Steps synchronized' : 'Synchronize steps to evolution phase'
              }
              aria-live="polite"
            >
              {syncSuccess ? (
        ...
        <div className="rounded-[2rem] bg-zinc-900 border border-white/5 p-6 space-y-4 shadow-xl">
          ...
          <div>
            <p className="text-2xl font-black text-white">
              {nutritionSummary?.totalCaloriesConsumed || 0}
            </p>
            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mt-1">
              Left: {nutritionSummary?.remainingCalories || 0}
            </p>
          </div>
          <div className="h-1.5 w-full bg-black rounded-full overflow-hidden">
            <motion.div
          ...
          <button
            ...
            className="w-full py-2 bg-white/5 rounded-xl text-[10px] font-black uppercase hover:bg-white/10 transition"
          >
            Log Meal
          </button>
        </div>
      ...
      <div className="space-y-4">
        <h3 className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.3em] px-2">
          Active Challenge
        </h3>
          ...
          {!activeQuest ? (
            <div className="text-center space-y-6 py-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-zinc-500 font-medium px-8 text-balance">
                  Complete today's routine to build your physique and rank up.
                </p>
              </div>
              <button
                onClick={handleStart}
                ...
                {activeQuest.exercises.map((ex) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`text-sm font-black ${ex.state === 'completed' ? 'text-zinc-500 line-through' : 'text-white'}`}
                      >
                        {ex.exerciseType.split('-').join(' ')}
                      </span>
                    </div>
                    <span className="text-[10px] font-bold text-zinc-500 uppercase">
                      {ex.targetReps || ex.targetDistance} {ex.targetReps ? 'reps' : 'm'}
                    </span>
                  </div>
                ))}
              </div>
