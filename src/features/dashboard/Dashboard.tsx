...
export function Dashboard({ onStartTraining, onViewNutrition }: DashboardProps) {
  ...
  useEffect(() => {
    ...
    if (isSynced) {
      timeoutId = setTimeout(() => setIsSynced(false), 2000);
    }
    return () => {
          ...
          <div className="rounded-[2rem] bg-zinc-900 border border-white/5 p-6 space-y-4 shadow-xl">
              ...
              <div className="flex gap-2">
                  <button
                    onClick={handleSync}
                    disabled={isSynced}
                    aria-label={isSynced ? "Steps synced" : "Sync steps to phase"}
                    className={`w-full py-2 rounded-xl text-[10px] font-black uppercase transition flex items-center justify-center gap-1 ${
                        isSynced
