...
export const MuscleGraphic = memo(
   ...
   * Memoizing MuscleGraphic with a custom comparator prevents unnecessary re-renders
   * when the parent component (e.g., WorkoutLogger) updates its state (like manual input)
   * and passes a newly-derived 'growth' object with the same values.
   *
   * IMPACT: Reduces re-renders from O(N) to O(0) during typing (where N is characters typed).
   */
  function MuscleGraphic({ growth }: { growth: MuscleGrowth }) {
  ...
  (prevProps, nextProps) => {
    // We only compare fields that actually affect the visual output of this component.
    // 'back' is excluded as it is not currently rendered in the SVG.
    return (
